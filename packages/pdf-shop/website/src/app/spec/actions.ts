...
import { createDocumentSpec } from '@/server/data/document-specs'

export type SpecActionState = {
  errors: Partial<Record<keyof DocumentSpecFormValues, { type: string; message: string }>>
...
export async function submitDocumentSpecAction(
  ...
  if (!parsed.success) {
    ...
    for (const [field, messages] of Object.entries(fieldErrors)) {
      if (messages?.[0]) {
        errors[field as keyof DocumentSpecFormValues] = { type: 'server', message: messages[0] }
      }
    }
